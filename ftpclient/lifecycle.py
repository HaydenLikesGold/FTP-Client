...
def send_retr_command(socket_file):
    ...
    socket_file.flush()
    ftp_response_handler.validate_retr_command(response.get_response(socket_file))
    ftp_response_handler.validate_retr_command(response.get_response(socket_file))
...
def close_server_connection(socket_file):
    ...
    socket_file.flush()
    ftp_response_handler.validate_pasv_and_quit_command(response.get_response(socket_file))
